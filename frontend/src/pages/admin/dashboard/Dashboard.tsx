...
import { Pill, CustomTooltip, KpiCard, fmt } from "./component"
import { useFetchAllOrders, useOrederMonthlyAnaytics } from "../../../context/api/order";
...
export default function AdminDashboard() {
  const { data: orderAnalytics, isLoading, error } = useOrederMonthlyAnaytics();
  const { data: order, isLoading: orderLoading } = useFetchAllOrders("limit=5");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <section className="lg:col-span-3 bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => navigate("/admin/order")} className="text-xs font-semibold text-blue-600 hover:underline">View all →</button>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {order?.orders.map((o) => (
                        <tr key={o._id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-3 pr-4 text-slate-700 font-medium whitespace-nowrap">{o.shippingAddress.fullName}</td>
                          <td className="py-3 pr-4 text-slate-500 whitespace-nowrap">{o.items[0].name}</td>
                          <td className="py-3 pr-4 font-semibold text-slate-800 whitespace-nowrap">{fmt.currency(o.totalAmount)}</td>
                          <td className="py-3">
