...
router.post("/", upload.single("thumbnail"),validateRequest(createThumbnailSchema),redisRateLimit({windowSec:60*15,max:10,prefix:"create-thumbnails-admin"}), asyncHandlerFunction(createThumbnail));
router.put("/:thumbnailId", upload.single("thumbnail"),validateRequest(updateThumbnailSchema),redisRateLimit({windowSec:60*15,max:20,prefix:"create-thumbnails-admin"}), asyncHandlerFunction(updateThumbnail));
